...
Implementação dos métodos da Bissecção, Newton-Raphson e Secante.
"""
...
def newton(f, df, x0, eps=1e-8, max_iter=200):
    ...
    for k in range(max_iter):
        ...
        if abs(dfx) < 1e-15:
            raise ValueError(
                f"Derivada nula ou extremamente próxima de zero em x_{k} = {xk}."
            )
...
def secante(f, x0, x1, eps=1e-8, max_iter=200):
    ...
    historico[i] contém k, x, fx, erro e 'avaliacoes_f' (contagem
    acumulada de chamadas a f).
    """
    g = contador(f)
    ...
    for k in range(max_iter):
        denom = (f1 - f0)
        if denom == 0.0:
            print(f"[secante] AVISO: denominador nulo (f(x1)=f(x0)) em x={x1:.10g}. Parando.")
            break
...
if __name__ == "__main__":
    ...
    r1, h1 = bisseccao(f, 0, 1)
    print("Bisseccao:", r1, "em", len(h1) - 1, "iterações\n")

    r2, h2 = newton(f, df, 0)
    print("Newton:   ", r2, "em", len(h2) - 1, "iterações\n")
